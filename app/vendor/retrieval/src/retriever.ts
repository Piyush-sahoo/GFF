...
export class Retriever {
  ...
  /** Record index in the lexical index, by id. */
  private readonly docIndexById: ReadonlyMap<string, number>;
  ...
  constructor(options: RetrieverOptions = {}) {
    ...
    this.docIndexById = new Map(records.map((r, i) => [r.id, i]));
  }
  ...
  async retrieveAsync(query: string, options: RetrieveOptions = {}): Promise<RetrievalResult> {
    ...
    const analysed = analyseQuery(query);
    const text = analysed.terms.map((t) => t.term).join(' ') || query;
    let queryVector: Float32Array | null = null;
    try {
      queryVector = await provider.embedQueryAsync(text);
    } catch {
      queryVector = null;
    }
  ...
  private embedQuery(analysed: ReturnType<typeof analyseQuery>): Float32Array | null {
    ...
    // External providers embed the natural-language query, minus filler.
    const text = analysed.terms.map((t) => t.term).join(' ');
    return this.dense.embedQuery(text || analysed.raw);
  }
