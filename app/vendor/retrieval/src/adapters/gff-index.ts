// @ts-nocheck -- upstream type error in @gff/retrieval, see sync-data.sh
/**
...
export async function loadGffIndexDense(
  ...
  let embedQuery: ((text: string, key: string) => Promise<number[]>) | null = null;
  ...
  if (!index || !options.embed) {
    ...
    const mod = await import(/* webpackIgnore: true */ libPath);
    if (!index) index = (await mod.loadIndex(indexDir, stem ? { stem } : {})) as GffIndexLike;
    embedQuery = mod.embedQuery as typeof embedQuery;
  }
  ...
  return new GffIndexDense(index!, embed ?? null);
}